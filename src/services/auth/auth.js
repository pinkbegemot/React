...
export const auth = {
   ...
   */
  getToken(fromStorage: Storage = APP_PERSIST_STORES_TYPES[0],
    tokenKey: TokenKey = TOKEN_KEY
  ): ?string {

    // alert("GetToken" +localStorage.getItem(tokenKey) );
  ...
  */
  setToken(
    ...
    // alert("setToken, token  " + value);
    if(!value || value.length <= 0) {
      alert("Token is empty" + value);
return;
    }
// localStorage:
if (toStorage === APP_PERSIST_STORES_TYPES[0]) {
  if (localStorage) {
    // alert("SetToken " + value);
    localStorage.setItem(tokenKey, value);
    // alert("toLocalstorage, tokenKey value " + value);
  }
}
// sessionStorage:
...
isAuthenticated(
  ...
  if (fromStorage === APP_PERSIST_STORES_TYPES[0]) {
    if ((localStorage && localStorage.getItem(tokenKey))) {
      //  alert( "From local storage "+localStorage.getItem(tokenKey))
...
/**
 * tell if token is expired (compared to now)
 *
 * @param {string} encodedToken - base 64 token received from server and stored in local storage
 ...
 *
 * @param {'localStorage' | 'sessionStorage'} [fromStorage='localStorage'] specify storage
 * @param {any} [userInfoKey='userInfoKey']  optionnal parameter to specify a token key
 * @returns {string} token value
 */
getUserInfo(
  fromStorage: Storage = APP_PERSIST_STORES_TYPES[0],
  ...
  // localStorage:
  if (fromStorage === APP_PERSIST_STORES_TYPES[0] && !this.isExpiredToken()) {
    // alert("getUserInfo "+ localStorage.getItem(USER_INFO));
    return (localStorage && (localStorage.getItem(userInfoKey))) || null; //TKJ
  }
  // sessionStorage:
  if (fromStorage === APP_PERSIST_STORES_TYPES[1]) {
...
getUserEmail(
  fromStorage: Storage = APP_PERSIST_STORES_TYPES[0],
  userInfoKey: UserInfoKey = EMAIL
): ?string {
  // localStorage:
  if (fromStorage === APP_PERSIST_STORES_TYPES[0]) {
    // alert("getUserInfoKey "+ localStorage.getItem(userInfoKey));
    return (localStorage && (localStorage.getItem(userInfoKey))) || null; //TKJ
  }
  // sessionStorage:
  if (fromStorage === APP_PERSIST_STORES_TYPES[1]) {
...
getScheme(): string { return this.scheme;},
...
setUserInfo(
  ...
  if (!value || value.length <= 0) {
    alert("LocalStorage, userInfo " + value);
  ...
  if (toStorage === APP_PERSIST_STORES_TYPES[0]) {
    if (localStorage) {
      // alert("before decode, value "+ stringify(value));
      const token = decode(value);
      console.log("Decode token " + JSON.stringify(token));
      localStorage.setItem(USER_INFO, token.given_name);
      localStorage.setItem(EMAIL, token.unique_name);
      // this.store.userName=token.given_name;
      // console.log( "to Store: "+this.store.userName);
      this.scheme = token.actort;
      //  alert("setUserInfoKey "+ token.given_name);
    }
  }
  // sessionStorage:
  if (toStorage === APP_PERSIST_STORES_TYPES[1]) {
